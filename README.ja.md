...
私の娘の雯雯(ウェンウェン)は小学校2年生です。毎日放課後帰ってくると、漢字の書き取り・算数の計算・英語の打卡…これらのタスク自体に問題はなく、子どももやる気があります。

しかし問題なのは、**日常のタスクをこなしてもポイントは付かないし、課題以上のことをしても何のフィードバックもない**ということです。长い間そうしていると、子どもの「自律性」が「型通りの生活」に少しずつ摩耗されていきます。

そこで私はポイント記録を始めました。賞罰のためではなく、**彼女が自分の「差分貢献」を見えるように**するためです。
...
| 次元 | V1 | **V2** |
|------|----|--------|
| 記帳方式 | キーワードマッチング(ルール固定) | **LLM セマンティック理解**(「雯雯今日数学+1ポイント」でも「雯雯今日表現良好」でもOK) |
...
```
@Bot  雯雯今日進んで机を片付け、お母さんの皿洗いも手伝った
...
```bash
# 完全なパイプライン(LLM 認識 + 記帳)
python3 runtime/cli.py "雯雯今日数学+1ポイント"
